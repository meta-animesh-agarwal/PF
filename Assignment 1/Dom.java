...
public class Dom {
	...
	public List<Element> findElementByClass(String className) {
		...
		return recurssiveElementByClass(this.element, className);
	}
	 ...
	 * 
	 */
	List<Element> recurssiveElementByClass(Element tag, String className) {
		...
		for(Element tagInList: ((CompositeElement) tag).tagList()) {
			...
			if(tagInList.isHasInstance()) {
				...
				list.addAll( recurssiveElementByClass(tagInList, className) );
			}
