...
function onload(){
	...
	//to check wheather there is already a object in local storage or not
	if(localStorage.getItem("ToDo") && localStorage.getItem("ToDo") != "null"){
		...
		//used to sort the list according to date
		sorts('Sort' , 4);
...
//changing the task from complete to current
...
//changing the task from current list to complete list
...
function sorts(sortName , type){
	...
	//used to clear both the complete and current lists
	clearall();
